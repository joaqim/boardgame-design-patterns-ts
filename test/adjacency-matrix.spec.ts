...
import { createAdjacencyMatrix } from "../src/containers";

describe("adjacency matrix", () => {
  it("initial empty", () => {
    let m = createAdjacencyMatrix({
      length: 4,
    });
  ...
  it("values can be get/set through index", ()=> {
    const zero = createAdjacencyMatrix({length: 4});
    const m = createAdjacencyMatrix({length: 4});
